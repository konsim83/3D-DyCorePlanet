  ...
  , timestep_number(0)
{
  TimerOutput::Scope timing_section(
    this->computing_timer, "BoussinesqModel - constructor and grid rescaling");
...
BoussinesqModel<dim>::setup_dofs()
{
  TimerOutput::Scope timing_section(this->computing_timer,
                                    "BoussinesqModel - setup dofs of systems");
  ...
  /*
   * Setup partitioners to store what dofs and matrix entries are stored on the
   * local processor
   */
...

  {
    ...
    DoFTools::extract_locally_relevant_dofs(nse_dof_handler, nse_relevant_set);
  ...
  /*
   * Setup constraints and boundary values for NSE. Make sure this is consistent
  ...
  Assembly::CopyData::TemperatureRHS<dim> &             data)
{
  ...
  typename DoFHandler<dim>::active_cell_iterator nse_cell(&this->triangulation,
                                                          cell->level(),
                                                          cell->index(),
                                                          &nse_dof_handler);
...
template <int dim>
BoussinesqModel<dim>::Postprocessor::Postprocessor(const unsigned int partition)
  : partition(partition)
{}
