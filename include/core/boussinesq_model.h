 ...
 *
 * Implements a bouyancy Boussinesq model in non dimensional form on a rotating
 * spherical shell, i.e., a Navier-Stokes-Model with temperature forcing:
 * @f{eqnarray*}
 ...
 * number and \f$\hat A=L_{\mathrm{ref}}u_{\mathrm{ref}}/A\f$ is something like
 * a varying the P\'eclet number. This is necessray if the thermal diffusivity
 * is spatially varying and has high contrast which makes it difficult to
 * identify a single constant number like the P\'eclet number to determine if
 * the a regime under consideration is either dominated by convective effects or
 * by diffusive effects globally. \f$\rho(T)=1-\beta(T-T_{\mathrm{ref}})\f$ is a
 * dimensionless scaling factor and \f$\hat g\f$ is the scaled gravity
 * acceleartion given by
 * @f{eqnarray*}{
 * \hat g = \frac{L_{\mathrm{ref}}}{u_{\mathrm{ref}}^2}g
 * @f}
 *
 * Quantities marked with \f$(\cdot)_{\mathrm{ref}}\f$ are reference quantities.
 */
template <int dim>
...
private:
  ...
  void
  setup_temperature_matrices(const IndexSet &temperature_partitioning,
...
extern template class BoussinesqModel<2>;
extern template class BoussinesqModel<3>;
